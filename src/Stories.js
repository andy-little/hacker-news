...
const Stories = () => {
  ...
  return <section className="stories">
    {hits.map(({author, created_at, num_comments, objectID: id, title, url, points})=>{
      if (title && url){
        ...
        <p className="info">Author: <span>{author} |</span> {num_comments} comments | {points} points </p>
        <a className="read-link" href={url} target="_blank" rel="noopener noreferrer">read more</a>
        <button className="remove-btn" onClick={() => hideStory(id)}>hide</button>
      </article>
      }
      else{
        return
